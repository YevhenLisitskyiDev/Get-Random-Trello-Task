...
const TOKEN =
  "ATTAe301af5e461dba099c8f9115ac445bd7a273b796f007bdd921fba80105fc9ce3B6226645";

const trello = new Trello(API_KEY, TOKEN);

export function createTrelloCard(cardData) {
  const { boardName, status, title, tags, deadline, description, members } =
    cardData;
  ...
  trello.get("/1/members/me/boards", function (err, userBoards) {
    ...
    trello.get(
      `/1/boards/${selectedBoard.id}/lists`,
      function (err, boardLists) {
        if (err) throw err;

        const selectedList = boardLists.find((list) => list.name === status);

        trello.get(
          `/1/boards/${selectedBoard.id}/labels`,
          function (err, labels) {
            if (err) throw err;

            const selectedLables = labels
              .filter((label) =>
                tags.map((tag) => Object.values(tag)[0]).includes(label.name)
              )
              .map((label) => label.id);
...
export function sendRandomTask(msg, bot) {
  trello.get("/1/members/me/boards", function (err, userBoards) {
    if (err) throw err;

    const selectedBoards = userBoards.filter(
      (board) =>
        board.name === "Regular Tasks" || board.name === "One-Time Tasks"
    );

    trello.get(
      `/1/boards/${sample(selectedBoards)?.id}/cards`,
      function (err, boardCards) {
        if (err) throw new Error(title);
